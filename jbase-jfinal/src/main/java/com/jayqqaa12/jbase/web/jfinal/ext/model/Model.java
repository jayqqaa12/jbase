...
//	}
	...
	public List<M> findByCache(String sql) {
		loadTableName();
		return super.findByCache(TABLENAME, SecKit.md5(sql), sql);
	}
	...
	public long getCount(String sql, Object... params) {
		loadTableName();
		if (sql.contains("select")) sql = TxtKit.split(sql.toLowerCase(), "from")[1];
		if (sql.contains("order by")) sql = TxtKit.split(sql, "order by")[0];
