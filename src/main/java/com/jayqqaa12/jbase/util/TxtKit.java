...
public final class TxtKit
{
	...
	public static int containSum(String source, String dest)
	{
		...
		return (source.length() - new TxtKit(source).restr(dest, "").toString().length()) / dest.length();
	}
	 ...
	 * String log2 = TxtKit.inject(log, "name", "ziquee", "无此字段");
	 * //执行结果如下：
	 ...
	 * <strong>下面是一个示例性的代码：</strong><p>
	 * String log = "对不起，要更新的字段出错。字段名:?, 字段值:?, 错误原因：?";
	 * String log2 = TxtKit.injectBy(log, "?", "name", "ziquee", "无此字段");
	 * //执行结果如下：
	 * log2="对不起，要更新的字段出错。字段名:name, 字段值:ziquee, 错误原因:无此字段";
	 * 
	 * @param inputString
	...
	public static String[] splitDef(String str,String sp){
		...
		if(TxtKit.split(str, sp).length==0){
			
			return new String[]{str};
		}
		else return TxtKit.split(str, sp);
	}
	...
	public static String htmlToText(String inputString)
	{
		...
		{
			System.err.println("->TxtKit.htmlToText(String inputString) ERROR:" + e.getMessage());
		}
	...
	public static double parseDouble(String s, double defV)
	{
		try
		{
			return Double.parseDouble(new TxtKit(s).replace("^(\\d+)(\\.*)(\\d*)(.|\r|\n)*$", "$1$2$3").toString());
		} catch (Exception e)
		{
			return defV;
		}
	...
	public static int parseInt(String s, int defV)
	{
		try
		{
			return Integer.parseInt(new TxtKit(s).replace("(\\d+)(.|\r|\n)*$", "$1").toString());
		} catch (Exception e)
		{
			return defV;
		}
	...
	public static long parseLong(String s, long defV)
	{
		try
		{
			return Long.parseLong(new TxtKit(s).replace("(\\d+)(.|\r|\n)*$", "$1").toString());
		} catch (Exception e)
		{
			return defV;
		}
	 ...
	 */
	public TxtKit replace(String regStr, String s1)
	{
		Pattern pattern = Pattern.compile(regStr);
		Matcher matcher = pattern.matcher(str);
	 ...
	 */
	@SuppressWarnings("unused")
	public TxtKit replaceFirst(String regStr, String s1)
	{
		Pattern pattern = Pattern.compile(regStr);
		Matcher matcher = pattern.matcher(str);
	 ...
	 */
	public TxtKit restr(String s1, String s2)
	{
		StringBuilder sb = new StringBuilder();
